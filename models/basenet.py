...
class ResBottle(nn.Module):
    def __init__(self, option='resnet18', pret=True):
        ...
        self.features = resnet50(pretrained=pret)
        self.bottleneck = nn.Sequential(
            nn.Linear(self.features.out_features, bottleneck_dim),
...
class ResBottle_back(nn.Module):
    def __init__(self, option='resnet18', pret=True):
        super(ResBottle, self).__init__()
        if option == 'resnet18':
        ...
        self.bottleneck = nn.Linear(model_ft.fc.in_features, 1024)
...
class ResClassifier_(nn.Module):
    def __init__(self, num_classes=12, num_layer=2, num_unit=2048, prob=0.5, middle=1000):
        super(ResClassifier, self).__init__()
        layers = []
